...
#intrinsic = #iree_gpu.mma_layout<MFMA_F8E4M3FNUZ_16x16x32_F32>
...
//===----------------------------------------------------------------------===//
  ...
  transform.named_sequence @__attention_main(%variant_op: !transform.any_op {transform.readonly}) {
    ...
    // transform.print %attention : !transform.any_op

    // Tile and distribute to workgroups
    // ==========================================
    // %padded, %pad, %copy_back = transform.structured.pad %attention {
    //   padding_values=[0.0 : f16],
    //   padding_dimensions=[1]
    // } : (!transform.any_op) -> (!transform.any_op, !transform.any_op, !transform.any_op)
    // transform.print %copy_back : !transform.any_op
    ...
    // Breakage.
    // %reducfor0 = transform.get_operand %forall_reduce[0] : (!transform.any_op) -> !transform.any_value
    // %reducfor0op = transform.get_defining_op %reducfor0 : (!transform.any_value) -> !transform.any_op
    // transform.print %reducfor0op : !transform.any_op
    // %infor0 = transform.get_operand %loop1[0] : (!transform.any_op) -> !transform.any_value
    // %infor0op = transform.get_defining_op %infor0 : (!transform.any_value) -> !transform.any_op
    // transform.print %infor0op : !transform.any_op
    // transform.print %loop1 : !transform.any_op
    // transform.print %forall_reduce : !transform.any_op
    ...
    %layout16x16x16 = transform.param.constant #intrinsic -> !transform.any_param
